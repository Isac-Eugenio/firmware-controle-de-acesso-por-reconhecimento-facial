...
CREATE TABLE IF NOT EXISTS `usuarios` (
    `id` INT(8) UNSIGNED NOT NULL UNIQUE,  -- ID de 8 dígitos aleatório e único
...
CREATE TABLE IF NOT EXISTS `historico` (
    ...
    CONSTRAINT `historico_ibfk_3` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE CASCADE,
...
DELIMITER //
CREATE TRIGGER before_insert_usuarios_id
BEFORE INSERT ON usuarios
FOR EACH ROW
BEGIN
    DECLARE new_id INT;
    SET new_id = FLOOR(10000000 + RAND() * 90000000);
    WHILE EXISTS (SELECT 1 FROM usuarios WHERE id = new_id) DO
        SET new_id = FLOOR(10000000 + RAND() * 90000000);
    END WHILE;
    SET NEW.id = new_id;
...
VALUES ('000.000.000-00', 'root', 'root', 'root.debug@gmail.com', '123456', '@Isac1998', 'docente', '0,0,0,0,0,0');
...
-- Inserir um registro no histórico (ajustado para usar usuario_id)
INSERT INTO historico (usuario_id, nome, alias, email, matricula, auth, mac, ip, local, esp_id, trust, data_acesso, horario_acesso)
SELECT id, nome, alias, email, matricula, auth, '00:14:22:01:23:45', '192.168.0.1', 'Laboratorio 1', 'ESP32-123456', 100, CURRENT_DATE, CURRENT_TIME
FROM usuarios WHERE email = 'root.debug@gmail.com';
